...
create or replace procedure P_PrlBA_Baoxiao_doApp(p_EntGid    varchar2, --企业Gid
...
begin
  ...
  if p_AppAssign <> '终止' then
    --插入审批人
    insert into wf_PrlBA_Baoxiao_App
      (EntGid,
       ModelGid,
       FlowGid,
       gid,
        ...
        from (select v.PostGid  AppGid,
              ...
              select o.AppGid, o.AppCode, o.AppName, 40 AppOrder, 40 AppType
               ...
               where o.EntGid = p_EntGid
                 ...
                 and v_AppFee > 50000
              union
              select o.AppGid, o.AppCode, o.AppName, 40 AppOrder, 45 AppType
                from v_wf_model_usr_app o
               where o.EntGid = p_EntGid
                 and o.ModelGid = p_ModelGid
                 and replace(lower(o.Modelcode), lower(v_ModelCode), '') in
    ...
    commit;
    --取出审批人中重复的审批人
    delete from wf_PrlBA_Baoxiao_App f
     where f.EntGid = p_EntGid
       and f.FlowGid = p_FlowGid
       and f.Apporder > 0
